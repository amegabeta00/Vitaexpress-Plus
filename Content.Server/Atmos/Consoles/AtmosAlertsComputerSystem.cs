...
public sealed class AtmosAlertsComputerSystem : SharedAtmosAlertsComputerSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        if (_updateTimer >= UpdateTime)
        {
            ...
            while (query.MoveNext(out var ent, out var entConsole, out var entXform))
            {
                ...
                if (HasComp<ActivatableUIRequiresPowerCellComponent>(ent)
                    && TryComp<PowerCellDrawComponent>(ent, out var draw))
                {
                    if (_cell.HasActivatableCharge(ent, draw) || _cell.HasDrawCharge(ent, draw))
                        Beep(ent, entConsole, highestAlert);
                }
                else if (HasComp<ActivatableUIRequiresPowerComponent>(ent))
                {
    ...
    private void Beep(EntityUid ent, AtmosAlertsComputerComponent entConsole, AtmosAlarmType highestAlert)
    {
        ...
        if (entConsole.NextBeep >= _gameTiming.CurTime
